...
Pracownik chce włączyć swoje zmiany do `master`. W tym celu musi zaktualizować swoje repo.
...
Gałęzie `upstream/master` i `mojezmiany` się rozgałęziają ponieważ pracownik i
szef pracowali równolegle.

Przepisujemy zmiany w `mojezmiany` nad `upstream/master`

```{r, engine.env=pr("pracownik")}
git clone fork-pracownik.git pracownik2
cd pracownik2
git remote add upstream ../projekt.git
git fetch upstream
git lgm
